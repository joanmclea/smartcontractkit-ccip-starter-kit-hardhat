...
contract ProgrammableTokenTransfers is CCIPReceiver, OwnerIsCreator {
  ...
  function sendMessage(
    ...
    // Send the message through the router and store the returned message I
  ...
  {
    if (index >= receivedMessages.length) revert IndexOutOfBound(index, receivedMessages.length - 1);
