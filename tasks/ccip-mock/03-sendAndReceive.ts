...
task(`mock-send-receive`, `sends message to CCIP receiver on your local dev chain`)
  .addOptionalParam(
    "sender",
  ...
  .addOptionalParam("message", "The text message to send via CCIP", "", types.string)
  .addOptionalParam(
    `tokenAddress`,
  ...
  .addOptionalParam(`amount`, `The amount of token to be transferred in units (eg wei, satoshi, etc))`, "0")
  .addOptionalParam(
    `feeTokenAddress`,
  ...
  .setAction(async (taskArgs: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    const MESSAGE = {
      ...
      transferToken: taskArgs.tokenAddress || hre.ethers.constants.AddressZero,
      transferAmount: taskArgs.amount.toString(),
      feeTokenAddress: taskArgs.feeTokenAddress || hre.ethers.constants.AddressZero,
    };

    const receiverStartingBal = await tokenToSend.balanceOf(taskArgs.receiver);
    ...
    try {
      ...
      if (parseInt(transferredAmount.toString()) !== parseInt(taskArgs.amount)) {
        throw Error(
      ...
      const receiverUpdatedBal = await tokenToSend.balanceOf(taskArgs.receiver);
      console.log(`Receiver's starting balance: ${receiverStartingBal}. Ending balance: ${receiverUpdatedBal}.`);
    } catch (error: any) {
      console.log(error);
